...
# Import các module counting với TensorRT support
try:
    from app.web.line_counting_web import LineCounterWeb
    from app.web.zone_counting_web import ZoneCounterWeb
    print("✅ Imported web modules with TensorRT support")
except ImportError as e:
    try:
        # Fallback to root modules
        from line_counting_web import LineCounterWeb
        from zone_counting_web import ZoneCounterWeb
        print("⚠️ Using root web modules (no TensorRT support)")
    except ImportError as e2:
        print(f"❌ Lỗi import web modules: {e}, {e2}")
...
def get_available_models():
    """API để lấy danh sách models có sẵn"""
    models_dir = os.path.join(os.path.dirname(__file__), 'models')
    available_models = {'pytorch': [], 'tensorrt': []}
    
    if os.path.exists(models_dir):
        for file in os.listdir(models_dir):
            if file.endswith('.pt'):
                available_models['pytorch'].append(file)
            elif file.endswith('.engine'):
                available_models['tensorrt'].append(file)
    
    return {'models': available_models}

@app.route('/api/quantize/<model_name>/<precision>')
def quantize_model_endpoint(model_name, precision):
    """API để quantize model thành TensorRT engine"""
    import subprocess
    import threading
        ...
        try:
            ...
            if result.returncode == 0:
                socketio.emit('quantization_complete', {
                    ...
                    'model': model_name,
                    'precision': precision,
                    'message': f'✅ TensorRT engine created: {model_name.replace(".pt", f"_{precision}.engine")}'
                })
            else:
                socketio.emit('quantization_complete', {
                    'success': False,
    ...
    return {
        'status': 'started',
        'message': f'🚀 Starting quantization: {model_name} -> {precision}'
    }
